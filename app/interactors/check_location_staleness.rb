...
class CheckLocationStaleness
  ...
  def call
    ...
    if context.user.location_updated_at.blank?
      ...
      context.fail!
    elsif context.user.location_updated_at < 8.hours.ago
      puts 'STALE MESSAGE WARN'
      context.message.reply do |reply|
        reply.text = "Going to update your data, but your location data is on the verge of becoming stale. When you get an opportunity please ."
        reply.send_with(context.bot)
      end
    end
  end
